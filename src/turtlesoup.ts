...
export function chordLength(radius: number, angleInDegrees: number): number {
  angleInDegrees = (angleInDegrees * Math.PI)/180;
  let sin : number = Math.sin(angleInDegrees/2);
  let chord : number = 2*radius*sin;
 // Round to avoid floating-point precision issues
  return Math.round(chord * 1000000) / 1000000;// Placeholder - replace with your implementation
}
...
export function distance(p1: Point, p2: Point): number {
  let sum: number = (p1.x-p2.x)**2 + (p1.y - p2.y)**2
  let dist:number = Math.sqrt(sum)
...
export function findPath(turtle: Turtle, points: Point[]): string[] {
  ...
  for (let index = 0; index < points.length - 1; index++) {
    let b: number = distance(points[index], points[index + 1]);
    result.push("forward " + b);

    if (index < points.length - 2) {
      let a: number = distance(points[index], points[index + 2]); 
      let c: number = distance(points[index + 1], points[index + 2]); 

      let angleRad: number = Math.acos((b * b + c * c - a * a) / (2 * b * c));

      let angleDeg: number = angleRad * (180 / Math.PI);

      result.push("turn " + angleDeg);
    }
  }
